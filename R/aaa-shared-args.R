...
#' @param allow_zero_length `(length-1 logical)` Are zero-length vectors acceptable?
#' @param min_size `(length-1 integer)` The minimum size of the object. Object size will be
#'   tested using [vctrs::vec_size()].
#' @param max_size `(length-1 integer)` The maximum size of the object. Object size will be
#'   tested using [vctrs::vec_size()].
...
#'   instead of "must match the regex pattern `{regex}`". If a complex regex
#'   pattern throws an error, try installing the stringi package.
#' @param x The argument to stabilize.
#' @param x_arg `(length-1 character)` An argument name for x. The automatic value will work
#'   in most cases, or pass it through from higher-level functions to make error
#'   messages clearer in unexported functions.
#' @param x_class `(length-1 character)` The class name of `x` to use in error messages. Use
